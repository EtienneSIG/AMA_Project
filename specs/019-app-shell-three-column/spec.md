...
- **FR-003b**: The profile control MUST be available and identical on **every page of every app** (including home/landing and secondary pages that lack their own `openProfile()`). The shell renders a **single shell-native editable profile modal** (first/last name, age, social handle, preferred language) loaded from `GET /api/auth/me` and saved via `PATCH /api/auth/me`, with Sign out / Close / Save. Clicking the panel identity header always opens this same modal — no page-to-page redirect and no duplicate avatar in the actions row.
