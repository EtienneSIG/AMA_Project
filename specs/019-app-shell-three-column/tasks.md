...
## Phase 1: Setup
- [ ] T001 EdTech Program Orchestrator: append the app-shell redesign scope + touched files to each app's README (Accountable: agents/edtech-program-orchestrator.chatmode.md) — **EXISTING: append.**
...
- [ ] T015 [US3] Demo Deployment Agent: responsive drawers (rail/panel collapse) with 0 overflow down to 360 px in shell.css/js (Accountable: agents/demo-deployment-agent.chatmode.md)
- [ ] T016 [US3] Demo Deployment Agent: accessibility (landmarks, focus order, WCAG AA, reduced-motion) + RTL mirroring + 014 theme variants in shell.css/js (Accountable: agents/demo-deployment-agent.chatmode.md)
- [ ] T017 [P] [US3] Cross-Agent QA Verifier: a11y + responsive + RTL + theme checks per app in demo/scripts/ (Accountable: agents/cross-agent-qa-verifier.chatmode.md)
