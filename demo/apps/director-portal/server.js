...
app.get('/api/reporting/metadata', (req, res) => {
  ...
  res.json({
    ...
    reporting: {
      loaded: cfg.loaded,
      path: cfg.path,
      defaultAggregationLevel: cfg.defaultAggregationLevel,
      reportCount: cfg.reports.length,
      fabric: cfg.fabric,
      error: cfg.error
    },
    reports: cfg.reports
...
app.get('/api/reporting/embed/:reportId', (req, res) => {
  const cfg = loadReportingConfig();
  const report = cfg.reports.find(r => String(r.reportId) === String(req.params.reportId));
